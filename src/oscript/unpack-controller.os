...
Процедура ОтправитьНаУдаленныйУзел()
	...
	Попытка
		...
		Результат = Распаковщик.ВыполнитьGitPush(Параметры.КаталогВыгрузки, Параметры.GitURL, "master", Параметры.GitПользователь, Параметры.GitПароль);
...
Функция ПолучитьПараметрыИзОкружения()
	...
	Параметры.Вставить("GitПользователь", Окружение["git_user"]);
	Параметры.Вставить("GitПароль", Окружение["git_password"]);
