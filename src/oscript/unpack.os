...
Функция РежимОтладки(Знач УстановитьОтладку = Неопределено)
	...
	ТекущийРежим = мКонтекстВыполнения.РежимОтладки;
	Если УстановитьОтладку <> Неопределено Тогда
		мКонтекстВыполнения.РежимОтладки = УстановитьОтладку;
	КонецЕсли;
...
Процедура Инициализация(Знач Параметры) Экспорт
	...
	мКонтекстВыполнения.Вставить("РежимОтладки", Ложь);
...
Функция СоздатьВременныйКаталог()
	...
	Путь = СоздатьУникальныйКаталог("v8u");
...
Функция ВыгрузитьМодули(ИмяКаталогаБазы, Пользователь, Пароль, резКаталогВыгрузкиВсехМодулей83) //Экспорт
	...
	ПутьКаталогаВыгрузкиДанных = СоздатьУникальныйКаталог("v8e");
...
Процедура Тест_ДолженВыполнитьПолнуюВыгрузкуЧерезПараметрыЗапуска() Экспорт
	...
	лКаталогВыгрузки = СоздатьВременныйКаталог();
