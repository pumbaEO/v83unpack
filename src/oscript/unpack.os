...
Функция КлючЗапускаВременнойБазы()
	...
	КлючВременнойБазы = Неопределено;
	...
	Возврат КлючВременнойБазы;
	
КонецФункции

Функция ЗапуститьКомандуКонфигуратора(Знач ПереченьПараметров, Знач СтрогоПереданныеПараметры = Ложь)

	Если Не СтрогоПереданныеПараметры Тогда
	...
	Иначе
		РабочиеПараметры = ПереченьПараметров;
	КонецЕсли;
