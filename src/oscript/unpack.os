...
Функция ПолучитьТаблицуВерсийИзФайлаХранилища1С(ПутьКФайлуХранилища1С, резПутьКХМЛФайлуВерсийХранилища1С, резПутьКХМЛФайлуПользователейХранилища1С, резТаблицаИсторииХранилища, резТаблицаАвторовХранилища) Экспорт
	...
	Для каждого строка Из таблицаИсторииХранилища Цикл
		//таблицаПользователейХранилища.Найти(строка.ГУИД_Автора, "ГУИД_Автора");
...
Функция ПолучитьТаблицуВерсийИзФайлаХранилищаГит(КаталогВыгрузки, ПутьКФайлуХранилища1С, резТаблицаИсторииХранилища, резТаблицаАвторовХранилища, резТаблицаПользователейГит, резТекущаяВерсия) Экспорт
	...
	Для каждого строка Из таблицаАвторовХранилища Цикл
		...
		строкаПользователя = НайтиЗаписьВТаблице(таблицаПользователейГит, строка.ГУИД_Автора, "ГУИД_Автора");
		Если строкаПользователя = Неопределено Тогда
		...
		Иначе
			...
			НайденныеСтроки = НайтиСтрокиВТаблице(ТаблицаИсторииХранилища,строка.ГУИД_Автора,"ГУИД_Автора");
...
Функция ПрочитатьФайлВерсийГит(парамПутьКФайлуВерсий, Версия = "", ЗаписыватьПустой = Ложь)
	 ...
	 Пока ЧтениеXML.Прочитать() Цикл 
		 Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.Имя = "VERSION" Тогда 
			 //ВывестиОтладочноеСообщение("ЧтениеXML.Имя = "+ЧтениеXML.Имя);
...
Функция ЗаписатьФайлВерсийГит(парамПутьКФайлуВерсий, Версия =  "") 
	...
	Попытка
		Запись = Новый ЗаписьТекста(парамПутьКФайлуВерсий, "utf-8");
...
Функция ПрочитатьФайлАвторовГит(парамПутьКФайлуАвторов, ТаблицаПользователей =  Неопределено)
	 ...
	 Проверить(лФайл.Существует(), "Файл с авторов ГИТ <"+парамПутьКФайлуАвторов+"> не существует - Синхронизация хранилища");
	 ...
	 Исключение
		 ...
		 лРезультат = Ложь;
		 ВывестиОшибку(ОписаниеОшибки()); //, СтатусСообщения.Внимание);
	 КонецПопытки;
	 ОсвободитьОбъект(ТекстовыйФайл);
...
Функция ЗаписатьФайлАвторовГит(парамПутьКФайлуАвторов, ТаблицаПользователей)
	...
	Попытка
		ТекстовыйФайл = Новый ЗаписьТекста(парамПутьКФайлуАвторов,"utf-8");
...
Функция ПоНомеруВерсииСохранитьКонфигурациюИзХранилища1СИзБазыХранилища(ПутьКФайлуХранилища1С, НомерВерсииХранилища, резПутьКФайлуКонфигурации) Экспорт
	...
	лИмяФайлаВыгрузкиКаталог = СоздатьВременныйКаталог(); //СтруктураПараметров.ПутьКДанным;
	//СоздатьКаталог(лИмяФайлаВыгрузкиКаталог);
	
	путьTool_1CD = Новый Файл("bin\Tool_1CD.exe").ПолноеИмя;//КаталогВременныхФайлов()+"Tool_1CD." + (Новый УникальныйИдентификатор()) + ".exe";
...
Функция СформироватьURLПодключенияКGit(Знач URLРепозитария, Знач Пользователь, Знач Пароль)
	
	РазделительСхемыURL = "://";
	ДлинаРазделителя = СтрДлина(РазделительСхемыURL);
	
	ПозицияРазделителя = Найти(URLРепозитария, РазделительСхемыURL);
	Если ПозицияРазделителя = 0 Тогда
		ВызватьИсключение "Некорректно сформирован URL репозитария";
	КонецЕсли;
	
	Схема = Лев(URLРепозитария, ПозицияРазделителя + ДлинаРазделителя - 1);
	Адрес = Сред(URLРепозитария, ПозицияРазделителя + ДлинаРазделителя);
	
	РазделительАвторизацииИАдреса = "@";
	
	Если Не ПустаяСтрока(Пароль) Тогда
		ПарольДляURL = ":" + СтрЗаменить(Пароль, РазделительАвторизацииИАдреса, "%40");
	Иначе
		ПарольДляURL = "";
	КонецЕсли;
	
	СтрокаПодключения = Схема + Пользователь + ПарольДляURL + РазделительАвторизацииИАдреса + Адрес;
	ВывестиОтладочноеСообщение("URL для git-push: <"+СтрокаПодключения+">");
	
	Возврат СтрокаПодключения;
	
КонецФункции

Функция ВыполнитьGitPush(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки, Знач Пользователь, Знач Пароль) Экспорт

	URL = СформироватьURLПодключенияКGit(УдаленныйРепозиторий, Пользователь, Пароль);
	
	КомандныйФайл = СоздатьВременныйФайл(".bat");
	Запись = Новый ЗаписьТекста(КомандныйФайл, "cp866");
	
	Запись.ЗаписатьСтроку("cd /d """ + ЛокальныйРепозиторий + """");
	
	ИмяФайлаЛогаКоммита = СоздатьВременныйФайл("log");
	
	КомандаGit = " push -u " + СтрЗаменить(URL, "%", "%%") + " --all -v";
	
	строкаКоманды = """" + мПараметры.ПутьGit + """" + КомандаGit + " > " + ИмяФайлаЛогаКоммита + " 2>&1";
	сообщение = "строкаКоманды = <"+?(строкаКоманды = Неопределено, "Неопределено", строкаКоманды)+">";
	ВывестиОтладочноеСообщение(сообщение);
	Запись.ЗаписатьСтроку(строкаКоманды);

	Запись.ЗаписатьСтроку("exit /b %ERRORLEVEL%");
	Запись.Закрыть();
	
	Результат = ЗапуститьПакетныйФайл(КомандныйФайл);
	
	текст = "";
	ПолучитьТекстФайла(ИмяФайлаЛогаКоммита, текст);
...
Функция СинхронизироватьХранилищеКонфигурацийСГит(СтруктураПараметров) Экспорт
	...
	Иначе
		...
		Для Сч = 1 По ТаблицаИсторииХранилища.Количество()-1 Цикл
			...
			КонецЕсли;
		КонецЦикла;
		// ОВСЯНКИН - в псевдотаблице из массива структур нет сортировки
		// поэтому, максимальная версия не обязательно будет последней записью в таблице
...
Процедура ЗаписатьТаблицуПользователейВФайлАвторовGit(Знач ТаблицаПользователейХранилища, Знач ВыходнойФайл)
	...
	Попытка
		...
		Для Каждого Запись Из ТаблицаПользователейХранилища Цикл
			...
			ЗаписьФайла.ЗаписатьСтроку(Запись.ГУИД_Автора+";"+Запись.Автор+" <"+Запись.Автор+"@localhost>");
...
Функция ВыполнитьКоммитГит(КаталогРепозиторияГит, Комментарий, Автор, Дата=Неопределено) Экспорт 
	...
	ИмяФайлаВыполнения = СоздатьВременныйФайл("bat");
	...
	ФайлДляВыполнения.ЗаписатьСтроку(ПутьGit + " add .");
	...
	КонецЕсли; 
	СтрокаДляЗаписи = ПутьGit + " commit -a --file="""+ИмяФайлаКомментария+""" --author="""+Автор+""" >"+ИмяФайлаЛогаКоммита;
	...
	рез = ЗапуститьПакетныйФайл(ИмяФайлаВыполнения);
...
Процедура Тест_ДолженВыполнитьПолнуюВыгрузкуЧерезПараметрыЗапуска() Экспорт
	...
	// Овсянкин
	// Тест должен: 
	// 1. Проверить, что в КаталогВыгрузки нет файлов
	// 2. Получить тестовый файл из "макета"
	// 3. Разобрать этот файл на компоненты
	// 4. Разложить компоненты по папкам и разместить результат раскладывания в каталоге КаталогВыгрузки
	// 5. Проверить, что в КаталогВыгрузки попали файлы
...
Процедура ТестДолжен_ВыполнитьКоммитФайловВГит() Экспорт
	...
	// 2. Получить тестовый файл из "макета"
	// 3. Разобрать этот файл на компоненты
	// 4. Инициализировать репозиторий GIT в КаталогВыгрузки
	// 5. Разложить компоненты по папкам и разместить результат раскладывания в каталоге КаталогВыгрузки
	// 6. Добавить компоненты в scope git
	// 7. Выполнить коммит в гит
...
Процедура ТестДолжен_ВыполнитьGitPush() Экспорт
	...
	Пароль         = "put-your-password-if-you-wanna-test-this";
	...
	СтрокаУРЛ = СформироватьURLПодключенияКGit(URLРепозитария, Пользователь, Пароль);
	
	Запись.ЗаписатьСтроку("chcp 1251 > nul");
	Запись.ЗаписатьСтроку("cd /d """ + ЛокальныйРепозиторий+"""");
	Запись.ЗаписатьСтроку("""" + мПараметры.ПутьGit + """" + " clone " + СтрокаУРЛ + " ""%CD%""> " + ФайлЛога + " 2>&1");
	Запись.Закрыть();
	
	ЗапуститьПакетныйФайл(ФайлЗапуска);
	ЗавершитьПроцесс_TGitCache_exe();
	текст = "";
	ПолучитьТекстФайла(ФайлЛога, текст);
	Сообщить(текст);
	
	СоздатьФайлАвторовГит(ЛокальныйРепозиторий);
	...
	Успех = ВыполнитьGitPush(ЛокальныйРепозиторий, URLРепозитария, ИмяВетки, Пользователь, Пароль);
...
Процедура Тест_ДолженПодготовитьРепозитарийКСинхронизацииСХранилищем() Экспорт
	...
	СтрокиПроверки.Добавить("60248d56-c966-4648-8f43-6606d369fb67;Администратор <user@localhost>");
	СтрокиПроверки.Добавить("00902af9-0d35-4c8b-a6c0-381796159ec1;Отладка <user1@localhost>");
...
Процедура СоздатьФайлАвторовГит(Знач Каталог)
...
Процедура ТестДолжен_СинхронизироватьХранилищеКонфигурацийСГит() Экспорт	
	...
	РезультатИнициализацииГитЧисло = ИнициализироватьТестовоеХранилищеГит(лПутьКаталогаВыгрузкиДанных);
	...
	СоздатьФайлАвторовГит(лПутьКаталогаВыгрузкиДанных);
	юТест.ПроверитьИстину(Новый Файл(лПутьКаталогаВыгрузкиДанных+"\AUTHORS").Существует());
	ЗаписатьФайлВерсийГит(лПутьКаталогаВыгрузкиДанных + "\VERSIONS");
	ПроверитьСуществованиеФайлаКаталога(лПутьКаталогаВыгрузкиДанных+"\AUTHORS");
	...
	ИмяФайлКомандыПроверкиХранилища = СоздатьВременныйФайл("bat");
	...
	ФайлКомандыПроверкиХранилища.ЗаписатьСтроку(ПутьGit+" log --pretty=oneline >"""+ИмяФайлаЛогаГит+"""");
