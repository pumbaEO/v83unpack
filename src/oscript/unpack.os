...
Функция АбсолютныйПуть(Знач ОтносительныйПуть)
	Каталог = "."; //Каталог = Новый Файл(ТекущийСценарий().Источник).Путь;	
	Рез = Каталог + "\" + ОтносительныйПуть;
	Возврат Рез;
...
Процедура Инициализация(Знач Параметры) Экспорт
	...
	мКонтекстВыполнения.Вставить("ФайлПрограммыРаспаковки", Новый Файл(АбсолютныйПуть("bin\UnpackV8.exe")));
	...
	ФайлDLL = Новый Файл(АбсолютныйПуть("bin\zlib1.dll"));
...
Функция ВыгрузитьФайлВерсийИзФайлаХранилища1С(ПутьКФайлуХранилища1С, резПутьКХМЛФайлуВерсийХранилища1С, резПутьКХМЛФайлуПользователейХранилища1С, резПутьКТаблицамИзФайлаХранилища1С = Неопределено) Экспорт
	...
	ФайлTool1C = Новый Файл(АбсолютныйПуть("bin\Tool_1CD.exe"));
	...
	ЗапуститьПриложение(СтрокаЗапуска, лПутьКТаблицамИзФайлаХранилища1С, Истина);
	// ОВСЯНКИН
...
Функция ПоНомеруВерсииСохранитьКонфигурациюИзХранилища1СИзБазыХранилища(ПутьКФайлуХранилища1С, НомерВерсииХранилища, резПутьКФайлуКонфигурации) Экспорт
	...
	путьTool_1CD = Новый Файл(АбсолютныйПуть("bin\Tool_1CD.exe")).ПолноеИмя;//КаталогВременныхФайлов()+"Tool_1CD." + (Новый УникальныйИдентификатор()) + ".exe";
...
Процедура ЗаписатьТаблицуПользователейВФайлАвторовGit(Знач ТаблицаПользователейХранилища, Знач ВыходнойФайл)
	...
	Попытка
		...
		Для Каждого Запись Из ТаблицаПользователейХранилища Цикл
			...
			СтрСчетчик = ?(СчетчикПользователей = 0,"",Строка(СчетчикПользователей));
			ЗаписьФайла.ЗаписатьСтроку(Запись.ГУИД_Автора+";"+Запись.Автор+" <user"+СтрСчетчик+"@localhost>");
...
Функция ВыполнитьКоммитГит(КаталогРепозиторияГит, Комментарий, Автор, Дата=Неопределено) Экспорт 
	...
	ФайлДляВыполнения.ЗаписатьСтроку("echo %CD% > %tmp%\out.txt");
...
Процедура ТестДолжен_ВыполнитьКоммитФайловВГит() Экспорт
	...
	//Ф = Новый Файл("gitrepo");
	//КаталогВыгрузки = Ф.ПолноеИмя;
	// Sleep(1); // для возможности удаления файлов репозитария
	// БезопасноУдалитьВременныйФайл(КаталогВыгрузки);
