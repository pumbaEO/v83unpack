...
// для предкомпиляции. Удалить ---

Перем НаправлениеСортировки;
Перем ПутьКПлатформе1с;
Перем ПутьКПлатформе1с83;
Перем КодировкаТекста;
Перем ИспользоватьРежимОтложенногоЗапускаВыгрузкиМодулейИзКонфигуратора;
Перем ЗадержкаЗапускаВыгрузкиМодулейИзКонфигуратора;
Перем ДопустимаяДлина;
Перем РежимУправленияБлокировкойДанных;
Перем РежимБлокировкиДанных;
Перем ВыводитьСообщенияВЖурналРегистрации;
Перем УровеньЖурналаРегистрации;
Перем ВключенРежимОтладки;
...
Функция ПравильныйПутьКПлатформеПоВерсииПлатформы(версияПлатформы, парамОписаниеОшибки = "") Экспорт
	...
	версия1 = СписокСтрок[1];
	версия2 = СписокСтрок[2];
...
Процедура Инициализация() Экспорт
	...
	ФайлПрограммыРаспаковки = Новый Файл("bin\UnpackV8.exe"); //Новый Файл(файлКаталог.ПолноеИмя + "\UnpackV8.exe");
	...
	ПутьКБазе = СоздатьУникальноеИмяКаталога(РабочийКаталог + "tmpDB");// _ПолучитьИмяВременногоФайла(" ");
	ЗарегистрироватьВременныйФайл(ПутьКБазе);
...
Функция СоздатьПустуюБазу(ПутьКФайлуКонфигурации = "", резИмяКаталогаБазы = "") //Экспорт //СтруктураПараметров
	...
	БылаОшибка = Ложь;
			...
			// ОВСЯНКИН - oscript не поддерживает препроцессор, но он и не нужен здесь, т.к. это мертвый код.
			// #Если НЕ Клиент И НЕ Сервер Тогда
			// Если Ложь Тогда
				// СтруктураПараметров = Новый Структура; //Справочники.Конфигурации.СоздатьЭлемент();
			// КонецЕсли;
...
Функция ВыполнитьКоммитГит(КаталогРепозиторияГит, Комментарий, Автор, Дата=Неопределено) Экспорт 
	...
	ФайлДляВыполнения.ДобавитьСтроку("set GIT_AUTHOR_DATE="""+ДатаPOSIX(Дата)+"""");
...
Процедура ОбработатьПараметрыЗапуска(ПараметрЗапуска, СтруктураПараметров = Неопределено) Экспорт
	...
	Если ВыгрузитьДанные Тогда
		...
		//лКаталогВыгрузки = "";
		лИмяКаталогаБазы = "";
		успешно = РазложитьКонфигурацию1СПоПапкамСогласноИерархииМетаданных("", "", ПутьКФайлуКонфигурации, КаталогВыгрузки, лИмяКаталогаБазы); //СтруктураПараметров.КаталогВыгрузкиВсехМодулей83, СтруктураПараметров.КаталогВыгрузки);
...
Процедура СоздатьМассивВременныхПутейЕслиОнЕщеНеСоздан()
	Если _массивВременныхПутей = Неопределено Тогда
	//ОВСЯНКИН Если ТипЗнч(_массивВременныхПутей) <> Тип("Массив") Тогда
