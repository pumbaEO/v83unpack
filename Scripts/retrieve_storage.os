...
Процедура ПрочитатьНастройки()
	...
	ТекстАргументов = "";
	ВсегоАргументов = АргументыКоманднойСтроки.Количество()-1;
	Для Сч = 0 По ВсегоАргументов Цикл
		Значение = АргументыКоманднойСтроки[Сч];
		Если Сч = ВсегоАргументов Тогда // не показываем пароль в логе
			Значение = ?(ПустаяСтрока(Значение),"", "*******");
		КонецЕсли;
		
		ТекстАргументов = ТекстАргументов + Символы.ПС + " - " + Значение;
