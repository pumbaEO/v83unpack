...
Процедура Инициализировать(Знач МодульУправления) Экспорт
	...
	мМодульУправления = МодульУправления;
	мНастройки = мМодульУправления.Настройки();
...
Функция ЕстьРаботающиеСеансы(Знач Дескриптор, Знач ТихаяПроверка = Ложь, Знач ИгнорироватьКонсольКластера = Истина) Экспорт
	...
	ТекРежимВыдачиСообщений = мМодульУправления.ВыдаватьСообщенияСборки();
	мМодульУправления.ВыдаватьСообщенияСборки(Не ТихаяПроверка);
	...
	Попытка
		...
		ОсвободитьОбъектКластера(InfobaseDescriptor);
	Исключение
		мМодульУправления.ВыдаватьСообщенияСборки(ТекРежимВыдачиСообщений);
		ВызватьИсключение;
	КонецПопытки;
...
Процедура СообщениеСборки(Знач Сообщение)
	...
	мМодульУправления.СообщениеСборки(Сообщение);
