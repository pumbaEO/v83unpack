...
Процедура ОтключитьПользователей(Знач Дескриптор) Экспорт
	
	ЗаблокироватьСоединенияСБазой(Дескриптор);
...
Функция ЕстьРаботающиеСеансы(Знач Дескриптор) Экспорт
	
	InfobaseDescriptor = НайтиДескрипторИнформационнойБазы(Дескриптор.ServerAgent, Дескриптор.Cluster);
	КоличествоСеансов = Дескриптор.ServerAgent.GetInfoBaseSessions(Дескриптор.Cluster, InfobaseDescriptor).Количество();
	
	ОсвободитьОбъектКластера(InfobaseDescriptor);
	
	Возврат КоличествоСеансов > 0;
